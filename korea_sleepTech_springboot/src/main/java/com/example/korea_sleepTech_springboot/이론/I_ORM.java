...
public class I_ORM {
    ...
    // - 반복적인 SQL 작성 없이 데이터 조작 가능 (DBMS에 독립)
    ...
    * */
    ...
    // cf) ORM 에서 사용되는 주요 애너테이션
    // 1. @Entity
    // : 클래스를 DB 테이블과 매핑할 때 사용
    // +) name 옵션 추가
    //      : 엔티티 이름을 지정, 테이블명과 클래스명이 다를 경우 명시
    //      @ Entity(name="user_Table") // 테이블명은 User
    //      public class User {}
    ...
    //      - unique 옵션: 해당 필드의 값이 유일해야하는지 여부를 지정 (기본갑: false)
    ...
    // cf) JPA(ORM, 객체와 RDBMS와 연결) VS MyBatis(SQL Mapper, SQL 중심 접근)
